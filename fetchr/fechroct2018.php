...
function register_setting_options()
{
    ...
    register_setting( 'mena-settings-group', 'mena_token' );
    ...
    register_setting( 'mena-settings-group', 'is_auto_print');
    ...
    <div class="wrap">
        ...
        <form method="post" action="options.php">
            ...
            <table class="form-table">
                ...
                <tr valign="top">
                    <th scope="row">fetchr brear Token</th>
                    <td><input type="text" name="mena_token" value="<?php echo esc_attr( get_option('mena_token') ); ?>" /></td>
                </tr>
                <tr valign="top">
                ...
                <tr valign="top">
                    <th scope="row">Auto Print?</th>
                    <td>
                        <input name="is_auto_print" type="checkbox" value="1" <?php if (get_option('is_auto_print') == "1"): ?> checked="checked" <?php endif; ?> />
                    </td>
                </tr>
        ...
        <table>
            <tr>
                ...
                <td>
                    <form method="post" action="">
                      <input type="submit" name="unprinted_print" id="unprinted_print" class="button button-primary" value="Print Un-Printed Orders">
                    </form>
                </td>
            </tr>
        </table>
...
if (isset($_POST['unprinted_print']) ){
   // hit_mena_api();
     $args = array(
    ...
    foreach ($orders as $order_wc)
    {
        ...
        $printed = $order_wc->get_meta('printed_awb');
        if ($printed =="0") {
            print("<br>");
            $tracking_no = $order_wc->get_meta('awb');

            $file_url=get_awb_pdf($tracking_no);
            print(' | pdf:'.$file_url);
            if ($file_url=="No Orders Found"){
                if ( ! update_post_meta ($order_id, 'printed_awb', '0' ))
                {
                    add_post_meta($order_id, 'printed_awb', '0', true ); 
                }
            }else{

                if ( ! update_post_meta ($order_id, 'awb_link_mini', $file_url  ))
                {
                    add_post_meta($order_id, 'awb_link_mini', $file_url , true );
                }
      if (get_option('is_auto_print') == "1"){
            try {
                $printed_awb=print_awb($file_url,$order_id,$gcpl); 
                if( $printed_awb != 1){
                    $printed_awb = 0;
                    if ( ! update_post_meta ($order_id, 'printed_awb', $printed_awb ))
                    {
                        add_post_meta($order_id, 'printed_awb', $printed_awb, true ); 
                    }
                }else{
                    if ( ! update_post_meta ($order_id, 'printed_awb', $printed_awb ))
                    {
...
if (isset($_POST['test_print']) )
{
    ...
    function send_whatsapp($msg,$number){
        ...
        $datalist_wab = array(
            ...
            'token' => '883771ab52f8bdd83601a8280585c9365b66d0295b4ff',
            'uid' => '201285304127',
            'to' => ''.$number,
            'text' => $text
        );
    ...
    function updateOrderStatus(){
     $args = array(
        'status' => array('wc-fetchr-processing','wc-picked'),
    ...
    // print("<br><br>------------ Bulk Status <br>");
     ...
     foreach ($orders as $order)
     {
        ...
        if (!empty($meta_array)) {
            ...
            if ( ! update_post_meta ($order->get_id(), 'order status',$BulkStatus[$awb_no]))
            {
                ...
                print ('<br>Order: -> '.$order->get_id().'Status : ');
            ...
            if(strpos($BulkStatus[$awb_no], 'Delivered on') !== false){
                ...
                # code...
                $order->update_status( 'wc-cancelled-fetchr' );
            }elseif (strpos($BulkStatus[$awb_no], 'Created') !== false ) {
                # code...
                ...
                $created_orders[$order->get_id()]="".$order->get_status()." \t ".$BulkStatus[$awb_no]." \t ".$description." \t ".$order->get_date_created();
                ...
                if ($order->get_meta('whatsapp_followup',false,'view')){
                    ...
                    $datalist_wab = array(
                        ...
                        شركة الشحن : فيتشر 0225991000'.'
                        رقم تتبع الشحنه : '.$awb_no
                    );
     ...
     print('<hr><h1>Please Review Those Orders</h1>');
     echo '<pre>';
     print_r($created_orders);
     echo '</pre>';
    }
...
function register_erp_order_status()
{
    ...
    register_post_status(       'wc-ship-with-fetchr', array(
        ...
        register_post_status(       'wc-cancelled-fetchr', array(
            'label'                     => 'Returned/Cancelled at Fetchr',
            'public'                    => true,
            'exclude_from_search'       => false,
            'show_in_admin_all_list'    => true,
            'show_in_admin_status_list' => true,
            'label_count'               => _n_noop( 'Returned/Cancelled at Fetchr <span class="count">(%s)</span>', 'Returned/Cancelled at Fetchr <span class="count">(%s)</span>' )
        )
    );
...
function add_fetchr_processing_to_order_statuses( $order_statuses ) {
    ...
    foreach ( $order_statuses as $key => $status ) {
        ...
        if ( 'wc-processing' === $key )
        {
            ...
            $new_order_statuses['wc-cancelled-fetchr'] = 'Returned/Cancelled at Fetchr';
            // $new_order_statuses['wc-mahmoud-processing'] = 'Mahmoud Processing';
...
foreach ($products as $product) {
  ...
  $str_replace_with = array('\'\'', 'and','-');
    ...
    $headr[] = 'Authorization: '.get_option('mena_token');
    ...
    $so_no=$results->data[0]->so_no;

    print_r($results);
    print($results->data[0]->status);
    ...
    if ($results->data[0]->status == "success")
    {
        ...
        if ( ! update_post_meta ($order->ID, 'so_no', $so_no ))
        {
            add_post_meta($order->ID, 'so_no', $so_no, true );
        }

        if ( ! update_post_meta ($order->ID, 'awb_link', $results->data[0]->awb_link  ))
        {
            add_post_meta($order->ID, 'awb_link', $results->data[0]->awb_link , true );
        }
        ...
        if (get_option('is_auto_print') == "1"){
            // $file_url=get_awb_pdf($tracking_no);
            $file_url="https://cms-awbs.fetchr.us/mini_".$tracking_no.".pdf";
            print(' | pdf:'.$file_url);
            if ($file_url=="No Orders Found."){
                if ( ! update_post_meta ($order->ID, 'printed_awb', '0' ))
                {
                    add_post_meta($order->ID, 'printed_awb', '0', true ); 
                }
            }else{

                if ( ! update_post_meta ($order->ID, 'awb_link_mini', $file_url  ))
                {
                    add_post_meta($order->ID, 'awb_link_mini', $file_url , true );
                }
                ...
                try {
                    ...
                    if( $printed_awb != 1){
                        $printed_awb = 0;
                        if ( ! update_post_meta ($order->ID, 'printed_awb', $printed_awb ))
                        {
    ...
    function send_whatsapp_tracking($order_wc,$order,$tracking_no,$description,$adrress){
        $text='عزيزي '.$order_wc->shipping_first_name.' لقد تم شحن طلبك بنجاح -
             المتجات :'.$description.' 
             علي العنوان : '.$adrress.'
...
add_action('admin_footer-edit.php', 'custom_bulk_admin_footerr');

function custom_bulk_admin_footerr() {
  ...
  function add_fetchr_ship_actions_button( $actions, $the_order ) {
    ...
    //   $link_ref='https://www.waboxapp.com/api/send/chat?token=883771ab52f8bdd83601a8280585c9365b66d0295b4ff&uid=201285304127&to='.$customermobile.'&text='.urlencode($text);
